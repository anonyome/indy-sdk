...
class PoolUtils {
    static private let DEFAULT_POOL_NAME: String = "default_pool";
    static private let PROTOCOL_VERSION: Int = 2;

    static func createGenesisTxnFile(fileName: String, ledgerPoolIP: String) -> String {
        let genesisFile = EnvironmentUtils.getTmpPath(fileName: fileName)
        let testPoolIp = EnvironmentUtils.getTestPoolIP(ip: ledgerPoolIP)

        let defaultTxns: [String] = [
            String(format:
        ...
        // Write out the default data.
        let txnsArray = defaultTxns.joined(separator: "\n")
        do {
            try txnsArray.write(toFile: genesisFile.absoluteString, atomically: true, encoding: .utf8)
        } catch {
            print(error)
        }
    ...
    static func createPoolLedgerConfig(ledgerPoolIP: String) -> String {
    
        let genesisTxnFile: String = createGenesisTxnFile(fileName: "temp.txn", ledgerPoolIP: ledgerPoolIP);
        
//        PoolJSONParameters.CreatePoolLedgerConfigJSONParameter createPoolLedgerConfigJSONParameter = new PoolJSONParameters.CreatePoolLedgerConfigJSONParameter(genesisTxnFile);
